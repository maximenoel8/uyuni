...
When(/^I remove packages? "([^"]*)" from this "([^"]*)"((?: without error control)?)$/) do |package, host, error_control|
  ...
  if rh_host?(host)
    ...
    raw_output, _ = node.run(check_cmd, check_errors: false)
    # To avoid using | grep -v 'not installed' in command, use select to verify lane match the package to remove
